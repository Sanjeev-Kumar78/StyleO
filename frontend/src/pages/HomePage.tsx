...
const OUTFIT_ITEMS: OutfitItem[] = [
  { id: "mac",      name: "Waterproof Mac",   tag: "Outerwear", emoji: "🧥", compatible: true  },
  { id: "blazer",   name: "Navy Wool Blazer",  tag: "Jacket",    emoji: "🥼", compatible: true  },
  { id: "oxford",   name: "Oxford Shirt",      tag: "Top",       emoji: "👔", compatible: true  },
  { id: "trousers", name: "Slim Fit Trousers", tag: "Bottom",    emoji: "👖", compatible: true  },
  { id: "brogues",  name: "Leather Brogues",   tag: "Footwear",  emoji: "👞", compatible: true  },
  { id: "sneakers", name: "Canvas Sneakers",   tag: "Footwear",  emoji: "👟", compatible: false },
      ...
      >
        ...
        >
          <defs>
            ...
            <linearGradient id="bodyGradRight" x1="100%" y1="0%" x2="0%" y2="0%">
              <stop offset="0%"   stopColor="#12131A" />
              <stop offset="40%"  stopColor="#1B1C26" />
              <stop offset="100%" stopColor="#252735" />
            </linearGradient>
            ...
            <linearGradient id="goldPocketSquare" x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            <filter id="lapelShadowL" x="-30%" y="-20%" width="160%" height="160%">
              <feDropShadow dx="5" dy="6" stdDeviation="6" floodColor="#000" floodOpacity="0.45" />
            </filter>
            
            <filter id="lapelShadowR" x="-30%" y="-20%" width="160%" height="160%">
              <feDropShadow dx="-5" dy="6" stdDeviation="6" floodColor="#000" floodOpacity="0.45" />
            </filter>
          </defs>
          ...
          {/* Pocket Stitching */}
          <line x1="44" y1="115" x2="78" y2="105" stroke="rgba(255,255,255,0.12)" strokeWidth="1" strokeDasharray="2 2" />
          <line x1="31" y1="238" x2="75" y2="228" stroke="rgba(255,255,255,0.12)" strokeWidth="1" strokeDasharray="2 2" />
          <line x1="209" y1="238" x2="165" y2="228" stroke="rgba(255,255,255,0.12)" strokeWidth="1" strokeDasharray="2 2" />
...
const StateCard = ({
  ...
  state: (typeof GARMENT_STATES)[number];
}) => {
...
const HomePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      setSmooth((p) => ({ x: lerp(p.x, mouse.x, 0.06), y: lerp(p.y, mouse.y, 0.06) }));
        ...
        >
          ...
          >
            ...
            >
              ...
              <span className="hp-hero-eyebrow-text">
                StyleO
              </span>
            </motion.div>
            ...
            <h1 className="hp-hero-h1">
              Your Wardrobe,{" "}
              <em className="hp-hero-h1-em">Digitized.</em>
              <br />
              Your Style,{" "}
              <span className="hp-hero-h1-dim">Automated.</span>
            </h1>
            ...
            <div className="hp-hero-cta-row">
              ...
              <motion.button
                ...
                whileHover={{
                  scale: 1.04,
                  boxShadow: "0 0 48px var(--accent-glow), 0 0 0 1px var(--accent-border)",
                }}
                whileTap={{ scale: 0.97 }}
              >
                Digitize My Closet
              ...
              >
                <span className="hp-hero-cta-play-circle">
                  <svg viewBox="0 0 24 24" width="10" height="10" fill="var(--text-dim)">
        ...
        <div className="hp-lifecycle-grid">
          ...
          >
            ...
            <div className="flex items-center gap-3 mb-5">
              ...
              <span className="text-[0.7rem] tracking-[0.18em] uppercase text-[var(--accent)] font-[family-name:var(--font-body)]">
                Lifecycle Management
              </span>
            </div>

            {/* Heading */}
            <h2 className="font-[family-name:var(--font-display)] font-light text-[clamp(1.8rem,3.5vw,3.2rem)] text-[var(--text-primary)] leading-[1.15] mb-5">
              Not Just Suggestions.{" "}<em className="italic">Complete</em>{" "}Lifecycle Management.
            </h2>

            {/* Sub-text */}
            <p className="text-[1.05rem] leading-relaxed mb-10 max-w-[36rem] hp-lifecycle-subtext">
            ...
            <div className="flex flex-col gap-3">
              {GARMENT_STATES.map((s, i) => (
                <button
                  key={s.id}
                  onClick={() => { setActiveState(i); setPaused(true); }}
                  className="hp-lifecycle-pill"
                  style={
                    {
                      "--s-color": activeState === i ? s.color : "var(--text-dim)",
                      "--s-bg": activeState === i ? "var(--bg-elevated)" : "transparent",
                      "--s-border": activeState === i ? s.color : "var(--border-subtle)",
                      "--s-glow": activeState === i ? `0 6px 28px ${s.glow}` : "none",
                      "--s-x": activeState === i ? "8px" : "0px",
                      "--s-num-bg": activeState === i ? s.color : "var(--surface-2)",
                      "--s-num-color": activeState === i ? "var(--bg)" : "var(--text-dim)",
                    } as React.CSSProperties
                  }
                >
                  {/* Numbered bubble */}
                  <div className="hp-lifecycle-pill-num">
                    {i + 1}
                  </div>
                  <div>
                    <div className="text-[0.95rem] font-medium tracking-[0.02em]">{s.label}</div>
                    <div className="text-xs mt-0.5 hp-lifecycle-pill-status">{s.statusLine}</div>
                  </div>
                </button>
              ))}
            </div>
        ...
        <div className="hp-context-inner">
          ...
          >
            <div className="hp-eyebrow-row">
              ...
              <span className="hp-eyebrow-span">
                Context Intelligence
              </span>
            </div>
            <h2 className="hp-context-h2">
              Dressed for <em className="hp-context-h2-em">exactly</em> where you&apos;re going.
            </h2>
          </motion.div>
          ...
          <div className="hp-context-split-grid">
            ...
            <div className="hp-context-left-col">
                ...
                <div className="hp-weather-body">
                  ...
                  <div>
                    <div className="hp-weather-condition-row">
                      ...
                      <span className="hp-weather-condition-label">Heavy Rain</span>
                    </div>
                    <div className="hp-weather-tags">
                      {["Waterproof", "Layering", "Formal"].map((tag) => (
                        <span key={tag} className="hp-tag-accent">{tag}</span>
                      ))}
                    </div>
                  </div>
                </div>
              ...
              >
                <div className="hp-calendar-card-body">
                  ...
                  <div>
                    <p className="hp-calendar-event-title">Client Pitch — Series A</p>
                    <p className="hp-calendar-event-time">09:30 AM · Canary Wharf HQ</p>
                    <div className="hp-calendar-tags">
                      {["Formal", "Professional", "Indoor"].map((tag) => (
                        <span key={tag} className="hp-tag-warn">{tag}</span>
                      ))}
                    </div>
                  </div>
                </div>
              ...
              >
                <div className="hp-ai-bar-header">
                  <div className="hp-ai-icon-circle">
                    <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="var(--accent)" strokeWidth="1.5">
                  ...
                  <div>
                    ...
                    <p className="hp-ai-bar-subtitle">Waterproof + formal context — assembling outfit…</p>
                  </div>
                </div>
                <div className="hp-ai-bar-track">
                  <motion.div
                    className="hp-ai-bar-fill"
                    animate={contextInView ? { width: ["0%", "100%"] } : { width: "0%" }}
            ...
            <div>
              ...
              <div className="hp-rejected-wrap">
                <AnimatePresence>
                  {outfitVisible && incompatibleItems.map((item, i) => (
                    <motion.div
                      key={`rej-${item.id}`}
                      className="hp-rejected-item"
                      initial={{ opacity: 1, x: 0 }}
                      animate={{ opacity: 0, x: 240, rotate: 6 }}
                      transition={{ duration: 0.65, delay: 0.25 + i * 0.3, ease: "easeIn" }}
                    >
                      ...
                      <span>{item.name}</span>
                      <span className="hp-rejected-note">— incompatible with rain / formal</span>
                    </motion.div>
                  ))}
                </AnimatePresence>
              </div>
              ...
              <div className="hp-outfit-grid">
                {outfitVisible
                  ? compatibleItems.map((item, i) => (
                      <OutfitItemCard key={item.id} item={item} delay={i * 0.1 + 0.6} />
                    ))
                  : Array.from({ length: 6 }).map((_, i) => (
                      <div
                        key={i}
                        className="hp-skeleton-card"
                        style={{ ["--delay" as string]: `${i * 0.15}s` } as React.CSSProperties}
                      />
                    ))}
              </div>
            </div>
